...
if (isset($_GET["action"])) {
    ...
    switch ($action) {
        ...
        case 'checkIfAlreadyExists':
            if (!$user) {
                http_response_code(401);
                header("Location: /");
            }
            require("src/Controllers/checkIfAlreadyExists.php");
            break;

        case 'modifyUserInformations':
            if (!$user) {
                http_response_code(401);
                header("Location: /");
            }
            require("src/Controllers/modifyUserInformations.php");
            break;
